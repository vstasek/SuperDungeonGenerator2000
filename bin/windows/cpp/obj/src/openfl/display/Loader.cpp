...
Void Loader_obj::load( ::openfl::net::URLRequest request,::openfl::_system::LoaderContext context){
{
		...
		if ((tmp4)){
			HX_STACK_LINE(383)
			tmp5 = (request->contentType != HX_HCSTRING("","\x00","\x00","\x00","\x00"));
		}
		else{
			HX_STACK_LINE(383)
			tmp5 = false;
		...
		if ((tmp5)){
			...
			if (  ( _switch_1==HX_HCSTRING("swf","\x42","\xab","\x57","\x00"))){
				...
				HX_STACK_LINE(391)
				tmp7 = HX_HCSTRING("application/x-www-form-urlencoded","\x9e","\x61","\x91","\xfa");
			}
;
;
			HX_STACK_LINE(385)
			...
			HX_STACK_LINE(385)
			tmp8->contentType = tmp7;
		}
		else{
			HX_STACK_LINE(397)
			::openfl::display::LoaderInfo tmp6 = this->contentLoaderInfo;		HX_STACK_VAR(tmp6,"tmp6");
			HX_STACK_LINE(397)
			tmp6->contentType = request->contentType;
		}
		HX_STACK_LINE(402)
		bool tmp6 = (request->url != null());		HX_STACK_VAR(tmp6,"tmp6");
		HX_STACK_LINE(402)
		...
		if ((tmp7)){
			HX_STACK_LINE(402)
			int tmp9 = request->url.indexOf(HX_HCSTRING("http://","\x52","\x75","\xcd","\x5a"),null());		HX_STACK_VAR(tmp9,"tmp9");
			HX_STACK_LINE(402)
		...
		if ((tmp9)){
			HX_STACK_LINE(402)
			int tmp11 = request->url.indexOf(HX_HCSTRING("https://","\xcf","\xb4","\xae","\x3e"),null());		HX_STACK_VAR(tmp11,"tmp11");
			HX_STACK_LINE(402)
		...
		if ((tmp10)){
			HX_STACK_LINE(404)
			::openfl::net::URLLoader tmp11 = ::openfl::net::URLLoader_obj::__new(null());		HX_STACK_VAR(tmp11,"tmp11");
			HX_STACK_LINE(404)
			::openfl::net::URLLoader loader = tmp11;		HX_STACK_VAR(loader,"loader");
			HX_STACK_LINE(405)
			::String tmp12 = ::openfl::events::Event_obj::COMPLETE;		HX_STACK_VAR(tmp12,"tmp12");

			HX_BEGIN_LOCAL_FUNC_S2(hx::LocalFunc,_Function_2_1,::openfl::display::Loader,_g,::openfl::net::URLLoader,loader)
			int __ArgCount() const { return 1; }
			Void run(Dynamic e){
				HX_STACK_FRAME("*","_Function_2_1",0x5201af78,"*._Function_2_1","openfl/display/Loader.hx",405,0x7a569d9f)
				HX_STACK_ARG(e,"e")
				{
					HX_STACK_LINE(407)
			...
			HX_STACK_LINE(405)
			loader->addEventListener(tmp12, Dynamic(new _Function_2_1(_g,loader)),null(),null(),null());
			HX_STACK_LINE(410)
			::String tmp13 = ::openfl::events::IOErrorEvent_obj::IO_ERROR;		HX_STACK_VAR(tmp13,"tmp13");

			HX_BEGIN_LOCAL_FUNC_S1(hx::LocalFunc,_Function_2_2,::openfl::display::Loader,_g)
			int __ArgCount() const { return 1; }
			Void run(Dynamic e){
				HX_STACK_FRAME("*","_Function_2_2",0x5201af79,"*._Function_2_2","openfl/display/Loader.hx",410,0x7a569d9f)
				HX_STACK_ARG(e,"e")
				{
					HX_STACK_LINE(412)
					Dynamic tmp14 = e;		HX_STACK_VAR(tmp14,"tmp14");
					HX_STACK_LINE(412)
					_g->BitmapData_onError(tmp14);
				}
				return null();
			}
			HX_END_LOCAL_FUNC1((void))
			...
			HX_STACK_LINE(410)
			loader->addEventListener(tmp13, Dynamic(new _Function_2_2(_g)),null(),null(),null());
			HX_STACK_LINE(415)
			loader->set_dataFormat(::openfl::net::URLLoaderDataFormat_obj::BINARY);
			HX_STACK_LINE(416)
			::openfl::net::URLRequest tmp14 = request;		HX_STACK_VAR(tmp14,"tmp14");
			HX_STACK_LINE(416)
			loader->load(tmp14);
			HX_STACK_LINE(417)
			return null();
			...
			HX_STACK_LINE(423)
			::lime::_system::BackgroundWorker tmp11 = ::lime::_system::BackgroundWorker_obj::__new();		HX_STACK_VAR(tmp11,"tmp11");
			HX_STACK_LINE(423)
			::lime::_system::BackgroundWorker worker = tmp11;		HX_STACK_VAR(worker,"worker");

			HX_BEGIN_LOCAL_FUNC_S2(hx::LocalFunc,_Function_2_1,::lime::_system::BackgroundWorker,worker,::openfl::net::URLRequest,request)
			int __ArgCount() const { return 1; }
			Void run(Dynamic _){
				HX_STACK_FRAME("*","_Function_2_1",0x5201af78,"*._Function_2_1","openfl/display/Loader.hx",425,0x7a569d9f)
				HX_STACK_ARG(_,"_")
				{
					HX_STACK_LINE(427)
					::String path = request->url;		HX_STACK_VAR(path,"path");
					HX_STACK_LINE(430)
					int tmp12 = path.indexOf(HX_HCSTRING("?","\x3f","\x00","\x00","\x00"),null());		HX_STACK_VAR(tmp12,"tmp12");
					HX_STACK_LINE(430)
					int index = tmp12;		HX_STACK_VAR(index,"index");
					HX_STACK_LINE(432)
					bool tmp13 = (index > (int)-1);		HX_STACK_VAR(tmp13,"tmp13");
					HX_STACK_LINE(432)
					if ((tmp13)){
						HX_STACK_LINE(434)
						int tmp14 = index;		HX_STACK_VAR(tmp14,"tmp14");
						HX_STACK_LINE(434)
						::String tmp15 = path.substring((int)0,tmp14);		HX_STACK_VAR(tmp15,"tmp15");
						HX_STACK_LINE(434)
						path = tmp15;
					}
					HX_STACK_LINE(439)
					...
					Void run(::openfl::display::BitmapData bitmapData){
						HX_STACK_FRAME("*","_Function_3_1",0x520271b9,"*._Function_3_1","openfl/display/Loader.hx",439,0x7a569d9f)
						HX_STACK_ARG(bitmapData,"bitmapData")
						{
							HX_STACK_LINE(439)
							::openfl::display::BitmapData tmp15 = bitmapData;		HX_STACK_VAR(tmp15,"tmp15");
							HX_STACK_LINE(439)
							worker->__Field(HX_HCSTRING("sendComplete","\x21","\xd2","\x84","\xc0"), hx::paccDynamic )(tmp15);
						}
						return null();
					}
					HX_END_LOCAL_FUNC1((void))
					...
					Void run(){
						HX_STACK_FRAME("*","_Function_3_2",0x520271ba,"*._Function_3_2","openfl/display/Loader.hx",439,0x7a569d9f)
						{
							HX_STACK_LINE(439)
							::String tmp15 = ::openfl::events::IOErrorEvent_obj::IO_ERROR;		HX_STACK_VAR(tmp15,"tmp15");
							HX_STACK_LINE(439)
							worker->__Field(HX_HCSTRING("sendError","\x60","\x89","\x99","\x33"), hx::paccDynamic )(tmp15);
						}
						return null();
					}
					...
					HX_STACK_LINE(439)
					::openfl::display::BitmapData_obj::fromFile(tmp14, Dynamic(new _Function_3_1(worker)), Dynamic(new _Function_3_2(worker)));
				}
				return null();
			}
			HX_END_LOCAL_FUNC1((void))
			...
			HX_STACK_LINE(443)
			Dynamic tmp12 = this->BitmapData_onError_dyn();		HX_STACK_VAR(tmp12,"tmp12");
			HX_STACK_LINE(443)
			worker->__Field(HX_HCSTRING("onError","\x29","\x6a","\x67","\x09"), hx::paccDynamic )->__Field(HX_HCSTRING("add","\x21","\xf2","\x49","\x00"), hx::paccDynamic )(tmp12,null(),null());
			HX_STACK_LINE(444)
			Dynamic tmp13 = this->BitmapData_onLoad_dyn();		HX_STACK_VAR(tmp13,"tmp13");
			HX_STACK_LINE(444)
			worker->__Field(HX_HCSTRING("onComplete","\xf8","\xd4","\x7e","\x5d"), hx::paccDynamic )->__Field(HX_HCSTRING("add","\x21","\xf2","\x49","\x00"), hx::paccDynamic )(tmp13,null(),null());
			HX_STACK_LINE(445)
